      ...
      <div>
        ...
        <div v-for="(item,index) in htmlConList">
          <div class="container" :id="index+'img'">
            <div class="card" :style="{  backgroundImage: 'url(' + coolBackGroundsBgImg + ')' }">
...
import coolbackgrounds from "@/assets/sbpk.png";
import JSZip from 'jszip'
import { saveAs } from 'file-saver';
const emit = defineEmits([  "getPromptBaseData"])
import {message} from "ant-design-vue";
const isLoading = ref(false)
const coolBackGroundsBgImg = ref(coolbackgrounds)
...
onMounted(()=>{
  ...
  if(lineCardHeight.value === '0px'){
    ...
    lineCardHeight.value = Number(lineCardHeight.value.replace('px','')) - 45 + 'px'
